...
class MultipleMetricStrategy(
    ...
    def tick(k: Kline, history: Boolean = false): Unit = {
        ...
        if (currentPosition.isEmpty) {
            // 避免频繁开仓
            if (!pauseOpen) {
                val d = if (Vector(macdDir, deaDir, ma5Direction, ma20Direction).forall(_ == 1)) {
                    1
                } else if (Vector(macdDir, deaDir, ma5Direction, ma20Direction).forall(_ == -1)) {
                    -1
                } else {
                    0
                }
            ...
            if (Vector(macdDir, deaDir, ma5Direction, ma20Direction).count(_ == d) < 3) {
                this.synchronized {
                    closeCurrent()
                }
            }

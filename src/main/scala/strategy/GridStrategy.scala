...
case class Position(openTime: LocalDateTime, closeTime: Option[LocalDateTime] ,direction: Int, openAt: BigDecimal, closeAt: Option[BigDecimal])
...
class GridStrategy(historyLength: Int, tickPercent: Double, tickThreshold: Int) extends BaseStrategy with KlineMixin {
  ...
  def closeCurrent() = {
    ...
    closed.prepend(current.copy(closeTime= Some(klines(0).datetime),closeAt = Some(klines(0).close)))
    println(s"close: profit: ${(k.close - current.openAt) * current.direction} ${current.direction} ${k.datetime}, price ${k.close}")
  ...
  override def step(k: Kline, history: Boolean = false): Unit = {
    ...
    val direction = ((klines(0).close - klines(1).close).abs / (klines(0).close - klines(1).close)).intValue

    val futurePressure = Range(0, tickThreshold).map(tick => {
      val price = k.close * math.pow(tickPercent + 1, direction)
      val prs = pressureAtPrice(price)
      prs
    }).sum / tickThreshold

    if(currentPressure > futurePressure * 1.3) {
      // 如果有持仓， 则判断方向， 不一致需要反手
